...
import akka_cookbook.chapter8_stream.examples_akka_io.modularity.Throttler
...
/**
  * @author Xiangnan Ren
  */
class IntervalBasedThrottlerSpec extends FlatSpec with
  ParallelTestExecution with Matchers with IntervalBasedThrottlerTestKit with StreamTestingSandbox {
  ...
  it should "limit rate of messages - low frequency" in sandbox { deps =>
    ...
    val throttle = Throttler.create[Int](2.perSecond)
  ...
  it should "limit rate of messages - medium frequency" in sandbox { deps =>
    ...
    val throttle = Throttler.create[Int](100.perSecond)
  ...
  it should "limit rate of messages - high frequency" in sandbox { deps =>
    ...
    val throttle = Throttler.create[Int](2000.perSecond) // max 2000 messages per second
  ...
  it should "keep limits with slow producer" in sandbox { deps =>
    ...
    val throttle = Throttler.create[Int](1000.perSecond) // max 1000 messages per second
