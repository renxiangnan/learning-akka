package akka_cookbook.chapter7_remoting_and_clustering.sec9_cluster_sharding
import akka.actor.{ActorRef, ActorSystem, Props}
import akka.cluster.sharding.{ClusterSharding, ClusterShardingSettings}
import akka.pattern.ask
import akka.util.Timeout
import akka_cookbook.chapter7_remoting_and_clustering.sec9_cluster_sharding.TemperatureActor.{GetCurrentTemperature, Location, UpdateTemperature}
import common.PrivateMethodExposer
...
/**
  * This example aims to DISTRIBUTE THE ACTORS.
  ...
  */
object ClusterShardingApplication1 extends App {
  ...
  locations.foreach { location =>
    (temperatureActor ? GetCurrentTemperature(location)).onSuccess {
      case x: Double => println(s"Current temperature in $location is $x")
    }
  }
  ...
  def printTree(): Unit = {
    println()
    println("######### Current ActorSystem State #########")
    println(this)
    println()
  }
}
