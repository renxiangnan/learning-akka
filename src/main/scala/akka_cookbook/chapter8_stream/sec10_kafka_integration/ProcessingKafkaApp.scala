package akka_cookbook.chapter8_stream.sec10_kafka_integration
...
/**
  * @author Xiangnan Ren
  */
object ProcessingKafkaApp extends App {
  implicit val actorSystem: ActorSystem = ActorSystem("SimpleStream")
  implicit val actorMaterializer: ActorMaterializer = ActorMaterializer()

  val bootstrapServers = "localhost:9092"
  val kafkaTopic = "akka_stream_topic"
  val partition = 0
  val subscriptions = Subscriptions.assignment(new TopicPartition(kafkaTopic, partition))
  ...
  val runnableGraph = RunnableGraph.fromGraph(
    GraphDSL.create() { implicit builder =>
      ...
      val tickSource = Source.tick(0.seconds, 100.millisecond, "Hello from akka stream through ")
      ...
      val mapFromConsumerRecord = Flow[ConsumerRecord[Array[Byte], String]].
